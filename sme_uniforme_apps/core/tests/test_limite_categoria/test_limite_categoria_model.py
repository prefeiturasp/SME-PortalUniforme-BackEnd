...
def test_srt_model(limite_categoria):
    assert limite_categoria.__str__() == 'Kit Inverno Preço Máximo: R$ 100.50'


def test_meta_modelo(limite_categoria):
    assert limite_categoria._meta.verbose_name == "Limite e obrigatoriedade da categoria"
    assert limite_categoria._meta.verbose_name_plural == "Limites e obrigatoriedades de categorias"
...
def test_admin():
    ...
    assert model_admin.list_display == ('categoria_uniforme', 'preco_maximo', 'obrigatorio')
    assert model_admin.ordering == ('categoria_uniforme',)
    assert model_admin.list_filter == ('categoria_uniforme',)


def test_metodo_limites_por_categoria_as_dict(limite_categoria, limite_categoria_verao):
    limites_por_categoria = LimiteCategoria.limites_por_categoria_as_dict()
    esperado = {
        limite_categoria.categoria_uniforme: 100.50,
        limite_categoria_verao.categoria_uniforme: 50.00
    }
